...
class LaporanAngsuranUtangExport implements FromArray, WithEvents, ShouldAutoSize
{
    protected $from;
    protected $to;
    protected $status;
    ...
    public function __construct($from, $to, $status, $data, $totalAngsuran, $market)
    {
        $this->from = $from;
        $this->to = $to;
        $this->status = $status;
    ...
    public function array(): array
    {
        ...
        foreach ($this->data as $tanggal => $transaksiPerTanggal) {
            ...
            $rows[] = ['Nama Pengutang', 'Jumlah Bayar', 'Sisa Angsuran', 'Total', 'Dibayar Oleh', 'Status'];
            ...
            foreach ($transaksiPerTanggal as $trx) {
                $rows[] = [
                    $trx->debt->nama_pengutang,
                    'Rp ' . number_format($trx->jumlah_bayar, 0, ',', '.'),
                    'Rp ' . number_format($trx->sisa_angsuran, 0, ',', '.'),
                    'Rp ' . number_format($trx->debt->transaction->total, 0, ',', '.'),
                    $trx->dibayar_oleh,
