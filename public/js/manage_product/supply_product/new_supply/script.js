...
$(document).on("click", ".btn-delete", function () {
    ...
    var kode_barang = row.find(".kd-barang-field").text();

    // Hapus dari localStorage
    var supplyData = JSON.parse(localStorage.getItem("supply")) || [];
    supplyData = supplyData.filter((item) => item.kode_barang !== kode_barang);
    localStorage.setItem("supply", JSON.stringify(supplyData));

    // Hapus dari tampilan tabel
    row.remove();

    // Sembunyikan tombol simpan jika tidak ada data lagi
